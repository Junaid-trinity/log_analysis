...
class LogFilesAnalysisAgentLog2:
    def __init__(self):
        self.langfuse_client = LangfuseClient()
        self.openai_client = OpenAIClient()
        self.conversation = Conversation()
        self.token_limit = 100000  # Define the token limit for each chunk
    ...
    async def execute_agent(self, log_file_paths):
        ...
        async def process_data(log_file_paths):
            ...
            log_file_names = []
            for log_file_path in log_file_paths:
                log_file_names.append(log_file_path)
                log_processor = DataProcessing_Log(log_file_path)
                convert_task = log_processor.convert_log_to_markdown()
                tasks.append(convert_task)
            markdown_reports = await asyncio.gather(*tasks)
        ...
        async def error_analysis_genai(markdown_report):
            """Analyze the error messages in batches and return a list of JSON responses."""
            try:
                chunks = self._split_into_batches(markdown_report)
                responses = []

                for chunk in chunks:
                    messages = [
                        {"role": "system", "content": LOG_FILES_ANALYSIS_SYSTEM_PROMPT_LOGS},
                        {"role": "user", "content": f"Analyse the following error logs file: {chunk}"}
                    ]
                    response = await self.openai_client.chat_completion_request(
                        messages=messages,
                        trace_id=self.conversation.trace_id,
                        request_name="log file error analysis",
                        response_format=DetailedResponseFull
                    )

                    # Parse and store each response as an individual JSON object
                    try:
                        response_data = json.loads(response.message)
                        responses.append(response_data)  # Append valid JSON response
                    except json.JSONDecodeError:
                        # Log error but continue to next chunk
                        self.langfuse_client.log_and_raise_error(
                            "LogfileanalysisAgent_log -> error_analysis_genai: JSON decoding error occurred.",
                            self.conversation.trace_id
                        )
                        responses.append({"error": "Invalid JSON format in response"})

                return responses  # List of individual JSON responses

            except Exception as e:
                self.langfuse_client.log_and_raise_error(
                    f"LogfileanalysisAgent_log -> error_analysis_genai: An error occurred: {e}",
                    self.conversation.trace_id
                )

        # Process log files and get markdown reports
        markdown_reports, log_file_names = await process_data(log_file_paths)
        ...
        return list(zip(error_analysis_reports, log_file_names))  # Each report is a list of JSON responses

    def _split_into_batches(self, markdown_report):
        encoding = tiktoken.get_encoding("cl100k_base")
        tokens = encoding.encode(markdown_report)

        batches = []
        start = 0
        while start < len(tokens):
            end = min(start + self.token_limit, len(tokens))
            last_pipe_index = markdown_report.rfind("|", start, end)
            if last_pipe_index == -1:
                last_pipe_index = end
            chunk = markdown_report[start:last_pipe_index]
